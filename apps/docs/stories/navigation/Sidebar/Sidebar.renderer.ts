import {
  SidebarManager,
  SidebarCollapseManager,
} from '../../../../../packages/vanilla/src/components/Sidebar';
import { CollapseManager } from '../../../../../packages/vanilla/src/components/Collapse';
import { renderSidebarButton } from '../SidebarButton/SidebarButton.renderer';
import { renderSidebarLabel } from '../SidebarLabel/SidebarLabel.renderer';
...
export const renderSidebar = () => {
  const buttonExample = renderSidebarButton({
    variant: 'primary',
  ...
  const buttonExample2 = renderSidebarButton({
    variant: 'primary',
  ...
  const child1 = renderSidebarButton({
    variant: 'primary',
    label: `Sales Summary`,
    child: true,
    active: false,
  ...
  const child2 = renderSidebarButton({
    variant: 'primary',
    label: `Sales Trends`,
    child: true,
  ...
  const child3 = renderSidebarButton({
    variant: 'primary',
    label: `Item Sales`,
    child: true,
    childPosition: 'middle',
  ...
  const child4 = renderSidebarButton({
    variant: 'primary',
  ...
  const buttonExample3 = renderSidebarButton({
    variant: 'primary',
    label: `${shoppingCartIcon} <span class="hide-on-collapse">Products</span>`,
    withParent: false,
  });
  ...
  const label = renderSidebarLabel({ label: 'E-commerce', withParent: false })
  ...
  const label2 = renderSidebarLabel({ label: 'Shops', withParent: false })
