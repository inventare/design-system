...
import { CLASS_NAME_COLLAPSE, CLASS_NAME_COLLAPSED, CLASS_NAME_COLLAPSING, CLASS_NAME_HORIZONTAL, CLASS_NAME_SHOW, Collapse } from './Collapse'
...
describe('Collapse', () => {
  const renderCollapseBase = (horizontal: boolean = false) => {
    ...
    container.innerHTML = `
      ...
      <div id="${id}" class="collapse ${horizontal ? 'horizontal' : ''} show">${content}</div>
    `;
