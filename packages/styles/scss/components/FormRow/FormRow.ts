import { createFormControl, FormControlProps } from '../FormControl/FormControl';
...
export const createFormRow = ({
  ...
  for (const row of rows) {
    ...
    rowElement.appendChild(createFormControl(row));
  ...
  submitRowElement.appendChild(createButton({ variant: 'primary', outline: true, label: 'Go Back' }));
  submitRowElement.appendChild(createButton({ variant: 'primary', label: 'Send' }));
